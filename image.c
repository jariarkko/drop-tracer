...
image_model2image_ypixel(unsigned int x,
  ...
  switch (mat) {
  case material_air:
    pixels[3*(z * (model->zSize) + x)+0] = 0;
    pixels[3*(z * (model->zSize) + x)+1] = 0;
    pixels[3*(z * (model->zSize) + x)+2] = 0;
    break;
  case material_rock:
    phyatom_color(&rgb,atom);
    pixels[3*(z * (model->zSize) + x)+0] = rgb.r;
    pixels[3*(z * (model->zSize) + x)+1] = rgb.g;
    pixels[3*(z * (model->zSize) + x)+2] = rgb.b;
    break;
  case material_water:
    pixels[3*(z * (model->zSize) + x)+0] = 0;
    pixels[3*(z * (model->zSize) + x)+1] = 0;
    pixels[3*(z * (model->zSize) + x)+2] = 255;
    break;
  default:
    fatalu("unrecognised atom material type",(int)mat);
  }
